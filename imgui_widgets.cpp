...
bool ImGui::Keybind(const char* label, int* key, int* mode) {
    ...
    const float size = GetWindowWidth() - 57;
    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size, 30));
    ImRect mode_rect(window->DC.CursorPos + ImVec2(size - 10, 0), window->DC.CursorPos + ImVec2(size, 29));
    ImRect key_rect;
    ...
    const char* mode_name[3] = { "Hold", "Toggle", "Always" };
    mode_size = CalcTextSize(mode_name[*mode], NULL, true);
    ...
    it_anim->second.alpha = ImClamp(it_anim->second.alpha + (ImGui::GetIO().DeltaTime * 8.f * (IsPopupOpen(nametwo.c_str()) * popup_open ? 1.f : -1.f)), 0.f, 1.f);
    it_anim->second.mode_color = ImLerp(it_anim->second.mode_color, IsPopupOpen(nametwo.c_str()) ? ImColor(240, 240, 240) : hovered_mode ? ImColor(160, 160, 160) : ImColor(85, 85, 90), g.IO.DeltaTime * 12.f);
    it_anim->second.key_color = ImLerp(it_anim->second.key_color, g.ActiveId == id ? ImColor(240, 240, 240) : hovered_key ? ImColor(160, 160, 160) : ImColor(85, 85, 90), g.IO.DeltaTime * 12.f);

    if (!IsPopupOpen(nametwo.c_str()))
        it_anim->second.alpha = 0.f;

    window->DrawList->AddText(rect.Min + ImVec2(-1.4f, 3), GetColorU32(ImGuiCol_Text), label);
    window->DrawList->AddRectFilled(mode_rect.Min, mode_rect.Max, GetColorU32(colors::Combo), 5.f, ImDrawFlags_RoundCornersRight);
    window->DrawList->AddRectFilled(key_rect.Min - ImVec2(5, 0), key_rect.Max, GetColorU32(colors::Combo), 5.f, ImDrawFlags_RoundCornersLeft);
    window->DrawList->AddRect(key_rect.Min, mode_rect.Max, GetColorU32(ImGuiCol_Border), 5.f);
    window->DrawList->AddRectFilled(ImVec2(key_rect.Max.x, key_rect.Min.y), ImVec2(mode_rect.Min.x, mode_rect.Max.y), GetColorU32(ImGuiCol_Border));

    PushStyleColor(ImGuiCol_Text, GetColorU32(ImGuiCol_Text));

    RenderTextClipped(mode_rect.Min - ImVec2(0, 1), mode_rect.Max - ImVec2(0, 1), mode_name[*mode], NULL, NULL, ImVec2(0.5f, 0.5f));
    RenderTextClipped(key_rect.Min - ImVec2(3, 1), key_rect.Max - ImVec2(3, 1), buf_display, NULL, NULL, ImVec2(0.5f, 0.5f));

    PopStyleColor();

    if (hovered_mode && io.MouseClicked[0])
    {
        OpenPopup(nametwo.c_str());
        SetNextWindowPos(ImVec2(key_rect.Min.x, key_rect.Max.y + 5));
    }

    SetNextWindowSize(ImVec2(mode_rect.Max.x - key_rect.Min.x, 95.f));
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(3, 0));
    PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(6, 10));
    PushStyleColor(ImGuiCol_Border, GetColorU32(ImGuiCol_PopupBg));
    PushStyleColor(ImGuiCol_PopupBg, GetColorU32(ImGuiCol_PopupBg));

    if (BeginPopup(nametwo.c_str(), ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
    {
        ImGuiWindow* state_window = GetCurrentWindow();

        SetCursorPosY(GetCursorPosY() + 5);
        if (Selectable(mode_name[0], *mode == 0))
        {
            *mode = 0;
            CloseCurrentPopup();
        }
        if (Selectable(mode_name[1], *mode == 1))
        {
            *mode = 1;
            CloseCurrentPopup();
        }

        if (Selectable(mode_name[2], *mode == 2))
        {
            *mode = 2;
            CloseCurrentPopup();
        }
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? colors::InputText_Active : hovered ? colors::InputText_Hovered : colors::InputText, g.IO.DeltaTime * 6.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, *v ? colors::Checkbox_Active : hovered ? colors::Checkbox_Hovered : colors::Checkbox, g.IO.DeltaTime * 6.f);
    it_anim->second.check_mark = ImLerp(it_anim->second.check_mark, *v ? colors::CheckMark : hovered ? colors::Transparent : colors::Transparent, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? colors::Text_Active : hovered ? colors::Text_Hovered : colors::Text, g.IO.DeltaTime * 6.f);
    it_anim->second.check_paint = ImLerp(it_anim->second.check_paint, *v ? 15.0f : 0.0f, g.IO.DeltaTime * 15.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.background_color), true, style.FrameRounding);


    PushClipRect(total_bb.Min + ImVec2(7, 7), total_bb.Min + ImVec2(7 + it_anim->second.check_paint, 27), false);
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(7, 7), GetColorU32(it_anim->second.check_mark), 14);
    PopClipRect();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(-2, 25));
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w + 56, label_size.y + style.FramePadding.y * 5.5f));
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        ...
        window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2 + 30, bb.Max.y), GetColorU32(it_anim->second.background_color), style.FrameRounding);
    ...
    {
        RenderTextClipped(bb.Min + ImVec2(5, 5) + style.FramePadding, ImVec2(value_x2 + 40, bb.Max.y), preview_value, NULL, NULL);
        ImRotateStart();
        PushFont(ico_minimize);
        RenderTextClipped(bb.Min + ImVec2(w + 15, 5) + style.FramePadding, ImVec2(value_x2 + 40, bb.Max.y), "K", NULL, NULL);
        PopFont();
        ImRotateEnd(1.57f * it_anim->second.expand_rotation);
    }

    if (label_size.x > 0) RenderText(ImVec2(bb.Max.x - 56 - w, bb.Min.y - 32 + style.FramePadding.y), label);
    PopStyleColor();
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    {
        g.NextWindowData.SizeConstraintRect.Min.x = ImMax(g.NextWindowData.SizeConstraintRect.Min.x, w + 1);
    }
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w + 1, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, g.Style.WindowPadding.y - 16)); // Horizontally align ourselves with the framed text
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(0, 15));


    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w + 55, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    if ((frame_bb.Min.x + it_filing->second.value) < grab_bb.Min.x) {
        it_filing->second.speed = (grab_bb.Min.x - (frame_bb.Min.x + it_filing->second.value)) / 10.f;
        ...
        it_filing->second.speed = ((frame_bb.Min.x + it_filing->second.value - grab_bb.Min.x)) / 10.f;
        it_filing->second.value -= it_filing->second.speed / ImGui::GetIO().Framerate * 160.f;
    }


    it_filing->second.background_color = ImLerp(it_filing->second.background_color, IsItemActive() ? colors::Slider_Active : hovered ? colors::Slider_Hovered : colors::Slider, g.IO.DeltaTime * 6.f);
    it_filing->second.circle_radius = ImLerp(it_filing->second.circle_radius, IsItemActive() ? 8.5f : 7.0f, g.IO.DeltaTime * 8.f);
    it_filing->second.circle_color = ImLerp(it_filing->second.circle_color, IsItemActive() ? colors::Circle_SliderActive : hovered ? colors::Circle_SliderHovered : colors::Circle_Slider, g.IO.DeltaTime * 6.f);
    it_filing->second.car_color = ImLerp(it_filing->second.car_color, IsItemActive() ? colors::Car_Slider_Active : hovered ? colors::Car_Slider_Hovered : colors::Car_Slider, g.IO.DeltaTime * 6.f);
    it_filing->second.text = ImLerp(it_filing->second.text, IsItemActive() ? colors::Text_Active : hovered ? colors::Text_Hovered : colors::Text, g.IO.DeltaTime * 6.f);

    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 14), ImVec2(total_bb.Min.x + w + 55, total_bb.Min.y + 20), GetColorU32(it_filing->second.background_color), 30);

    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 14), ImVec2(frame_bb.Min.x + it_filing->second.value + 3, total_bb.Min.y + 20), GetColorU32(it_filing->second.car_color), 30);

    window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + it_filing->second.value + 8, total_bb.Min.y + 16.5f), it_filing->second.circle_radius, GetColorU32(it_filing->second.circle_color), 30);


    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_filing->second.text));

    RenderTextClipped(frame_bb.Min + ImVec2(0, -22), frame_bb.Max + ImVec2(0, -22), value_buf, value_buf_end, NULL, ImVec2(1.0f, 0.5f));

    if (label_size.x > 0.0f) RenderText(ImVec2(frame_bb.Max.x - w - 55, frame_bb.Min.y + style.FramePadding.y - 22), label);
    PopStyleColor();
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() + 57, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 6.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    if (label_size.x > 0) RenderText(ImVec2(frame_bb.Max.x - CalcItemWidth() - 57, frame_bb.Min.y - 32 + style.FramePadding.y), label);
    PopStyleColor();
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleColor(ImGuiCol_PopupBg, GetColorU32(colors::Picker_Active));
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(CalcItemWidth(), style.FramePadding.y * 5.0f));
    const ImRect total_bb(bb.Min, bb.Max);

    bool hovered, held;
    bool pressed = ButtonBehavior(total_bb, ImGui::GetID(label), &hovered, &held);
    ...
    if (it_anim == anim.end())
    {
        anim.insert({ ImGui::GetID(label), coloredit_state() });
        it_anim = anim.find(ImGui::GetID(label));
    }

    it_anim->second.text = ImLerp(it_anim->second.text, picker_active_window ? colors::Text_Active : hovered ? colors::Text : colors::Text, g.IO.DeltaTime * 6.f);
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        SetCursorPos(GetCursorPos() - ImVec2(40, 0));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size + ImVec2(CalcItemWidth() + 26, 4));
    ...
    float grid_step = ImMin(size.x, size.y) / 3.99f;
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max - ImVec2(0, 4), GetColorU32(col_rgb), grid_step, ImVec2(-grid_step + off, off), rounding, ImDrawFlags_RoundCornersRight);
    ...
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min + ImVec2(CalcItemWidth() + 21, 4), bb_inner.Max - ImVec2(0, 4), GetColorU32(col_source), grid_step, ImVec2(off, off), rounding);
        else
            window->DrawList->AddRectFilled(bb_inner.Min + ImVec2(CalcItemWidth() + 21, 4), bb_inner.Max - ImVec2(0, 4), GetColorU32(col_source), rounding);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            window->DrawList->AddRect(bb.Min + ImVec2(CalcItemWidth() + 21, 4), bb.Max - ImVec2(0, 4), GetColorU32(ImGuiCol_Border), rounding); // Color button are often in need of some sort of border
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, selected ? colors::Selectable_Active : hovered ? colors::Selectable_Hovered : colors::Transparent, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? colors::Text_Active : hovered ? colors::Text_Hovered : colors::Text, g.IO.DeltaTime * 6.f);

    window->DrawList->AddRectFilled(ImVec2(text_min.x + 6, text_max.y - 20), ImVec2(text_min.x + 8, text_max.y - 1), GetColorU32(it_anim->second.background_color), 24);
